...
20201217 v4.1   Option of reference trace calculated on low force cycle positions: set boolean selfreference=True
"""""
...
tdmsfilename="20191124-151720.tdms"          #tdmsfilename="20200903-144604.tdms"    tdmsfilename="20200826-120242.tdms"

tdmsfilename="20201203-194431.tdms"
...
for nf in range(nbead): fname[nf] = tdmsfilename; start[nf]=6*60*1000/20; stop[nf]=306*60*1000/20; load[nf]=nf==0  # stop[nf]=1600000
...
AnchorPointStateList=['low force'] # correction with AnchorPoint as determined by anchor point  'low force' or '0 force' or 'custom' ; or 'None': no anchor point 
...
def BuildZinterpol(Z, T, TE, NE, DE):      # builds power time trace form event times
    I=(NE=='SigGen.Power (%)')
    TP=TE[I]; DP=DE[I]
    print('BuildZinterpol:', len(TP), 'events') #   print(TP)
    Z1=np.zeros(len(T)); j1=0
    for i in range(1,len(TP)-1):
        j=int(np.where(T==TP[i])[0][0])
    #    Z1[j1:j]=Z[j1-1]; j1=j
        Z1[j1:j]=np.mean(Z[max(0, j1-1000): j1-1]); j1=j
    return Z1
...
#    dur2s=np.sort(np.append(Tupjump_Tuphigh, np.ones(countNoOpen)*TEhigh))
    ...
    if DisplaySurvival:
        ...
        ax.plot(x2, y2_fit0, c='k',alpha=0.5, label='kd 1param fit='+"%.6f" % (1/pEq[0]))
...
for ibead in range(nbead):        ## Loop on test beads
 ...
 #       AFSG.MakeGraphXYall(tdms_track, imax, 100, 100, fname[ibead], path1+OutputFolder, OutFormat, SaveGraph)
    ...
    # Z1df = pd.Series(Z1).interpolate(method='polynomial', order=3)
    Z1 = BuildZinterpol(Zu, T, TE, NE, DE)
    Z1[indlow] = Zu[indlow]
    #    Zulow=Zu[indlow]; Xlow=X[indlow]; Ylow=Y[indlow]
    ...
    Z1avg=Rollavg(Z1, NZrefavg*4, mode)     # rolling average of self reference
    if RefBeadZcorrection:
        if selfreference==True:
            print('Zcorrection by low force self reference' )
            Zc=Zu-Z1avg; Z0c=Zr0; Zec=Z0
            figname=refname+'_self reference'
            plt.figure(figname, figsize=(6,6), dpi=100); ax = plt.gca()
            ax.scatter(T, Zu, marker='.', c='b', alpha=0.5)
            ax.plot(T, Z1, c='b', alpha=0.2); ax.legend()
            ax.plot(T, Z1avg, c='r', alpha=0.5); ax.legend()
            if SaveGraph: plt.savefig(path1+OutputFolder+figname+OutFormat)
        else:
