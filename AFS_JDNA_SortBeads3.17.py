...
rapa = True
latexpath = '/home/laurent/DATA/Redaction/AFS-JDNA/Figures/'
...
if rapa: 
    AbName = 'Rapa'; outfolder = 'Out20220621_Rapa/'
    ...
    AbName = 'Nef'; outfolder = 'Out20220621_Nef/'
    ...
    for inb, nt in enumerate(n.values):
        ...
        if (SelectMolecule == False) or (MolecID in ListMolec):
         ...
         #           dfres = pd.DataFrame(data={'Cp1Lo_nm': [0], 'p1Lo_nm': [0], 'Cp2Lo_nm': [0], 'p2Lo': [0], 'PullAngle': [0], 'PullAngleC': [0], 'PullPhi': [0], 'PullPhiC': [0]} )
                ...
                ax3x.loglog(fbinsX, PbinsX, 'k-', marker=mx, ms=6 , c=col, label=str(power)+'%', alpha=0.3)
                ...
                if MolecID==IDshow1: ax3t.loglog(fbinsX, PbinsX, 'k-', marker=mx, ms=6 , c=col, label=str(power)+'%', alpha=0.3)
         ...
         #       u = df0['Tupjump_Tuphigh_mask_cycle'][innt].values
          ...
          #      print('nb dur= ', len(u));# print(uz); print(v); print(fname)
           ...
           # wdur = wdur[wdur>mindur]
            ...
            if MolecID==IDshow1:
                ax3t.set_xlabel("Frequency (Hz)");  ax3t.set_ylabel("PSD (nm²/Hz)"); ax3t.axis([0.05, 300, 1, 1e4])
            ...
            for ax3i in [ax3x, ax3y]:
                ax3i.set_xlabel("Frequency (Hz)");  ax3i.set_ylabel("PSD (nm²/Hz)"); ax3i.axis([0.05, 300, 1, 1e4])
...
if SelectMolecule:
  ...
  #      print(wkxgP); print(wlgP); print(BeadRad_i)
   #     print(list(zip(wkxgP,wlgP,BeadRad_i)))
        ...
        if (x in IDshow):
            axPfBtest.errorbar(wP-DeltaP, wfxgP, wdfxgP, c=c, fmt=symb+'-', label='x ID'+str(ix)) 
            axPfBtest.errorbar(wP+DeltaP, wfygP, wdfygP, c=c, fmt=symb+'-', markerfacecolor='none', label='y ID'+str(ix))
        ...
        if (x in IDshow): axPAtest.errorbar(wP, waPcorr, c=c, fmt=symb+'-', label='ID'+str(ix), alpha=0.5) 
        axPA.errorbar(wP, waPcorr, c=c, fmt=symb+'-', label='From Median', alpha=0.5) 
   #     axPA.errorbar(wP, waCP, c=c, fmt='s-', markerfacecolor='none', label='From Contour', alpha=0.5) 
    #    axPA.errorbar(wP, waP, c=c, fmt='s-', markerfacecolor='none', label='From Contour', alpha=0.5) 
     ...
     #   axRR.scatter(dfx['R'+xy][0], slopePf**(1/3), c=cFOV, alpha=0.5)
        ...
        for axBisi in [axBis, axBisAB, axL, axLB, axA, axNC, axFL, axPf, axPL, axPfB, axPk, axPkB, axPkT, axPD, axPDB, axPA, axPP, axDZ, axLZ]:
            if axBisi == axLB:
                ...
                axBisi.text(0.5, 1400, Annotation+':'+res , size=fontsize)
      ...
      #          if ShowAnnotation: axBisi.text(1, 5, 'ID '+str(x)+' FOV '+str(wfov[ix]) , size=fontsize)
            ...
            if (x in IDshow): axPfBtest.legend(); axPAtest.legend()
...
for ax2i in [ax2, ax2Bis]:
    ...
    ax2i.set_xlim(0,10)
    if rapa: ax2i.set_ylim(0.01,0.1); ax2.set_xlim(0,MaxForce+5)
ax2.set_xlabel('Bell Force (pN)')
if rapa:
    ax2Bis.set_xlim(0,1.0)
else: 
    ax2Bis.set_xlim(0,1.5)
ax2Bis.set_xlabel('Distance Barrier (nm)'); ax2Bis.set_ylabel('koff at zero force (1/s)')
...
ax2Ter6.set_xlim(1000,2400); ax2Ter6.set_xlabel('DNA Full Length (nm)'); ax2Ter6.set_ylabel('DNA persistence length (nm)'); ax2Ter6.set_ylim(0,15)
...
for ax2i in [ax2, ax4, ax2Ter, ax2Ter1, ax2Ter2, ax2Ter3, ax2Ter4, ax2Ter5, ax2Ter6, ax2Ter7, axRR, axRR1]: ax2i.legend(loc="lower right", fontsize=fontsize)
...
for fignamei in [figname2, figname2Bis, figname2Ter, figname2Ter1, figname2Ter2, figname2Ter3, figname2Ter4, figname2Ter5, figname2Ter6,
                 ...
                 'Power_Stiffness_Ter', 'Power Friction', 'Power_Friction_Bis', 'Spectrum_test'+str(IDshow), 'Force_test', 'Angle_test']:
    if SaveGraph: plt.figure(fignamei); plt.savefig(outpath+fignamei, transparent=False)

for fignamei in ['offrate_vs_force', 'koff0_vs_xBeta', 'Lp_vs_L0', 'Maturation', 'Power_Lengths', 'Force_Lengths_Bis', 'Power_Angles',
                 'No_Close_Fraction', 'Power_vs_Force_Bis', 'Power_Stiffness_Ter', 'Power_Friction_Bis', 'multispectrumx', 'multispectrumy',
                 'RDiffusionprecx', 'RDiffusionHistx', 'multisurvival2Bis', 'offrate_vs_forceBis', 'Spectrum_test'+str(IDshow)]:
