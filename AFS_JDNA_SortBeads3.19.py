...
rapa = True
latexpath = '/home/laurent/DATA/Redaction/AFS-JDNA/CurrentMS/Figures/'
...
if rapa: 
    AbName = 'Rapa'; outfolder = 'Out20220718_Rapa/'
    nrow=9; ncol=16; nrow2=3; ncol2=7; deltaf = 2; MinForce = 1; MaxForce = 19; minsize = 10; nmaxfit=50; SelectForce = False
    HighPowerMax = 35; DeltaP=0.2
else:
    AbName = 'Nef'; outfolder = 'Out20220718_Nef/'
    nrow=9; ncol=16; nrow2=3; ncol2=3; nf = 9; deltaf = 3.5; MinForce = 5; MaxForce = 35; minsize = 4
    HighPowerMax = 10.5; DeltaP=0.05
print('Outfolder: ', outfolder)
#inputpath0="/home/laurent/DATA/Experiences Diverses/AFS/Data/SelectionJim/"
inputpath0="/home/laurent/DATA/Redaction/AFS-JDNA/DataAFS/"
outpath = inputpath0+outfolder
if not os.path.exists(outpath): os.makedirs(outpath); print('Creates ',outpath)
...
dfstage_nef = pd.read_excel(inputpath0+'Stage Summary_Nef.xlsx')
...
dfstage_rapa = pd.read_excel(inputpath0+'Stage Summary_Rapa.xlsx')
...
startstr = ''; endstr = '.csv'
if rapa:
    folder_cycle = "Rapa_Cycle/"  #  previously  "Output_csv file_cycle_2021-11-30 rapa update/"
    ...
    folder_cycle = "Nef_Cycles/"   #   previously "Output_csv files_2021-11-03 and 20 nef/"
    folder_spectra = "Nef_Spectra/"   #   previously "Output_csv files_4.57_2022-04-13 Raw Spectra Nef/"
...
ListMolec_nogap = np.arange(Nmolec)
...
def survival(dur, TEhigh, countNoOpen, shift, refname, marker=None, tmaxsurvival = tmaxsurvival, mindur=0, ax=None, col='k', exportname=None, shortlegend=None):
# calculates, fits and plots survival curve until TEhigh/1000.-shift
    ...
    if DisplaySurvival:
        if shortlegend!=None: ax.scatter(dur2s, dur2s_y, marker=marker, c= col, alpha=alpha1, label=shortlegend)
        if shortlegend==None: ax.scatter(dur2s, dur2s_y, marker=marker, c= col, alpha=alpha1, label=refname)
    ...
    pEq=[1.,0.]
    if DisplaySurvival:
        if shortlegend==None: ax.plot(x2, y2_fit0, c='k',alpha=alpha1, label='t fit='+"%3.0f" % (pEq0[0]) )
        if shortlegend!=None: ax.plot(x2, y2_fit0, c='k',alpha=alpha1)
...
#    if pprint: print('Spectrum', ' k (pN/nm)=',"%.5f" %  (pEq[0]),' D (µm²/s)=',"%.3f" %  (pEq[1]*1.e-6), 'D/Dtheo=', pEq[1]*friction/kBT_pN_nm), 'fc (Hz)= ',"%.3f" %  fc)
    if pprint: print('Single Spectrum', ' k (pN/nm)=',"%.5f" %  (pEq[0]),' D (µm²/s)=',"%.5f" %  (pEq[1]*1.e-6), 'fc (Hz)= ',"%.3f" %  fc)
...
def GlobalSpecFit(dfstage, folder_spectra, MolecID, xy, step=1, listD=None):   # D, listk = GlobalSpecFit(dfstage, 1)
# dfstage0['R'+xy] = kBT_pN_nm/FaxenXY(dfstage0['Bead radius (nm)'], p1Zo)/(6*np.pi*1e-9*dfstage0['D'+xy+'global'])   
    ...
    params = Parameters()
    if step==1:
        pname = 'D'; params.add('D', value=0.15e6, min=0.01e6, max=1e6)
    if step==2 and listD==None:
        pname = 'R'; params.add('R', value=800, min=600, max=2000)
    if step==2 and listD!=None:
        pname = 'D';
        Davg = np.mean([x for x in listD if str(x) != 'nan']); print('Davg = ', Davg)
        params.add('D', value=Davg, vary=False)
    print('=================================================================')
    print('Global Fit Spec ', pname, ' ID:', MolecID, ' #powers=', npower, xy)
    listR = []; listdR = []
    listD = []; listdD = []
    datax = {}; listfile = []
    listk = []; listdk = []; listL = []
    for inb, nt in enumerate(n.values):
        ...
        if os.path.isfile(fullnameRes):
            ...
            print(' missing Res file'); L = 900 %np.nan
   #     L = 1000
        listL += [L]
        datax['L'+str(inb)] = L
        ...
        if os.path.isfile(fullnameSpec):
            ...
            fminFit_Hz, fmaxFit_Hz = recalc_fminmax(fmin_Hz, fmax_Hz, power, pprint=True)
            fr = dfraw['fx']; intfr = (fr>=fminFit_Hz)&(fr<=fmaxFit_Hz); frt=fr[intfr]     

      #      if ( pname == 'R' and (n_calib[inb][0] not in n[inb] ) ) or pname == 'D':
            if ( pname == 'D' ) or ( pname == 'R' and not rapa and (n_calib[inb][0] not in n[inb] ) ) or rapa:
                if not rapa: print('  specific calibration file')
                listfile += [1]
     ...
     #       if pname == 'R' and n_calib[inb][0] in n[inb]:
            if  pname == 'R' and not rapa and n_calib[inb][0] in n[inb]:
                print('  cyclic calibration file'); listfile += [0]    
        else:
    ...
    def TotalFit(param, x=None, aax=None):
        ...
        for inb in range(npower):
            if listfile[inb] == 1:
                inb0+=1
                if pname == 'D': mi = FitSpectrumGen(datax['frt'+str(inb)], param['k'+str(inb)], param['D'])
                if pname == 'R': mi = FitSpectrumGenR(datax['L'+str(inb)], datax['frt'+str(inb)], param['k'+str(inb)], param['R'])
    ...
    if sum(listfile)>1:
        ...
        if pname == 'D': D = out.params['D'].value; dD=out.params['D'].stderr
        if pname == 'R': R = out.params['R'].value; dR = out.params['R'].stderr
        for inb in range(npower):
            if listfile[inb] == 1:
                listk += [out.params['k'+str(inb)].value]
                listdk += [out.params['k'+str(inb)].stderr]
                if pname == 'R': D = kBT/(6*np.pi*1.e-9*R * FaxenXY(R, listL[inb]) ); dD = dR * D / R
                if pname == 'D': R = np.nan; dR = np.nan
         #       print(R, listL[inb], D)    
                listD += [D]; listdD += [dD]
                listR += [R]; listdR += [dR]
            else:
...
def recalc_fminmax(fmin_Hz, fmax_Hz, power, pprint=False):
    fminFit_Hz = fmin_Hz
    fmaxFit_Hz = fmax_Hz
    if power>=25: fminFit_Hz=1
  #  if rapa and MolecID>=MolecIDa_fmin_Hz_set and MolecID<=MolecIDb_fmin_Hz_set:
    if rapa:
          fminFit_Hz=fmin_Hz_set
          if power>=20: fminFit_Hz=0.5
          if power>=25: fminFit_Hz=1
          if power>=30: fminFit_Hz=5
    if not rapa: fminFit_Hz=1; fmaxFit_Hz=fmax_Hz
    if not rapa and fmax_Hz>100: fmaxFit_Hz=100
    if pprint:
        print(f' fmin_Hz= {fmin_Hz:3.2f} fmax_Hz={fmax_Hz:3.2f}')
        print(f' fminFit_Hz= {fminFit_Hz:3.2f} fmaxFit_Hz={fmaxFit_Hz:3.2f}')
    return fminFit_Hz, fmaxFit_Hz
...
def ReadFromdf_Multisurvival(dfstage, nrow, ncol, nrow2, ncol2, figname, pprint=False, minsize=1, mindur=0.5, mindz=50, maxdz=500):
    ...
    if rapa: figsize2 = (ncol*sizefig/2, nrow*sizefig/2)
    if not rapa: figsize2 = (ncol2*sizefig, nrow2*sizefig)
    fig2, axall2 = plt.subplots(nrow2, ncol2, num=figname+'Bis', figsize=figsize2, dpi=100)
    fig2A, axall2A = plt.subplots(nrow2, ncol2, num=figname+'BisA', figsize=figsize2, dpi=100)
    fig2B, axall2B = plt.subplots(nrow2, ncol2, num=figname+'BisB', figsize=figsize2, dpi=100)
    fig3x, axall3x = plt.subplots(nrow2, ncol2, num='multispectrumx', figsize=figsize2, dpi=100)
    fig3y, axall3y = plt.subplots(nrow2, ncol2, num='multispectrumy', figsize=figsize2, dpi=100)
    fig4z, axall4z = plt.subplots(nrow2, ncol2, num='multihistodzjump', figsize=figsize2, dpi=100)
    figSpectestname = 'Spectrum_test'+str(IDshow1); fig = plt.figure(figSpectestname, figsize=(6,6), dpi=100); ax3t = plt.gca()
    figdztestname = 'dz_test'+str(IDshow1); fig = plt.figure(figdztestname, figsize=(6,6), dpi=100); ax4t = plt.gca()
    ...
    for inb, nt in enumerate(n.values):
        ...
        if (SelectMolecule == False) or (MolecID in ListMolec):
            ...
            ax4z = axall4z[iID//ncol2, iID%ncol2]
         ...
         #           dfres = pd.DataFrame(data={'Cp1Lo_nm': [0], 'p1Lo_nm': [0], 'Cp2Lo_nm': [0], 'p2Lo': [0], 'PullAngle': [0], 'PullAngleC': [0], 'PullPhi': [0], 'PullPhiC': [0]} )
                ...
                mx = next(poolmarker2x)
                ax3x.loglog(fbinsX, PbinsX, 'k-', marker=mx, ms=6 , c=col, label=str(power)+'%', alpha=alpha1)
                ax3y.loglog(fbinsY, PbinsY, 'k-', marker=next(poolmarker2y), ms=6 , c=col, label=str(power)+'%', alpha=alpha1)
                fx = dfx['Xfbins_Hz']; intfx = (fx>=fminFit_Hz)&(fx<=fmaxFit_Hz); fxt=fx[intfx]
                fy = dfy['Xfbins_Hz']; intfy = (fy>=fminFit_Hz)&(fy<=fmaxFit_Hz); fyt=fy[intfy]
                

                                 
                if MolecID==IDshow1: ax3t.loglog(fbinsX, PbinsX, 'k-', marker=mx, ms=6 , c=col, label=str(power)+'%', alpha=alpha1)
          #          ax3t.loglog(fx, dfx['Pbins'], 'k.', c=colormap[iID+1], label='x '+str(power)+'%')
                ...
                p1Zo = 900; GuessDTheo = 150000; Dx=0; frictionXY=friction0 * FaxenXY(BeadRad_i, p1Zo)

                print('GuessDTheo (µm²/s) =', GuessDTheo*1e-6)
          ...
          #      fcg = (kxg+kyg)*( Dxg+Dyg )/4/(2*np.pi*kBT_pN_nm)
                ...
                if MolecID==IDshow1: ax3t.loglog(frt, FitPxx_specxrg, 'k-')
                if ShowIndivFit:
                    if fc_xr < fmax_Hz/2:
                        print('show individual fit x: fc=',fc_xr,' < fmax=', fmax_Hz/2)
                        ax3x.loglog(frt, FitPxx_specxr, 'r+')
                    else:
                        print("don't show individual fit x: fc=",fc_xr,' > fmax=', fmax_Hz/2)
                    if fc_yr < fmax_Hz/2:
                        print('show individual fit y: fc=',fc_yr,' < fmax=', fmax_Hz/2)
                        ax3y.loglog(frt, FitPxx_specyr, 'r+')
                    else:
                        print("don't show individual fit y: fc=",fc_yr,' > fmax=', fmax_Hz/2)
         ...
         #       u = df0['Tupjump_Tuphigh_mask_cycle'][innt].values
          ...
          #      print('nb dur= ', len(u));# print(uz); print(v); print(fname)
           ...
           # wdur = wdur[wdur>mindur]
            ...
            if MolecID==IDshow1:
                ax3t.set_xlabel("Frequency (Hz)");  ax3t.set_ylabel("PSD (nm²/Hz)"); ax3t.axis([0.1, 20, 1, 5e3]); #ax3t.axis([0.05, 300, 1, 1e4])
                ax3t.legend(title='ID '+str(iID+1)+' x', title_fontsize=fontsize, prop={'size': fontsize}, fontsize=fontsize, loc="upper right")
            for ax3i in [ax3x, ax3y]:
                ax3i.set_xlabel("Frequency (Hz)");  ax3i.set_ylabel("PSD (nm²/Hz)")
                if rapa: ax3i.axis([0.05, 300, 1, 1e4])
                if not rapa: ax3i.axis([0.5, 300, 0.1, 1e3])
                if iID%ncol2!=0: ax3i.axes.get_yaxis().set_visible(False)
                if iID//ncol2!=nrow2-1: ax3i.axes.get_xaxis().set_visible(False)
            ax3x.legend(title='ID '+str(iID+1)+' x', title_fontsize=fontsize, prop={'size': fontsize}, fontsize=fontsize, loc="upper right")
            ax3y.legend(title='ID '+str(iID+1)+' y', title_fontsize=fontsize, prop={'size': fontsize}, fontsize=fontsize, loc="upper right")
            if len(wdur) >= minsize:
                ...
                for ax2i in [ax2, ax2A, ax2B, ax4z]:
                ...
                zrange = np.arange(300)
        #        ax4z.hist(wdz, bins=30, range=(0,300), label=); ax4z.axis([0,300, 0,20])
           #     sns.histplot(wdz, binwidth=10, binrange=(0,300), kde=True, color=col, ax=ax4z,  marker=marker, fill=False, label=shortlegend)
                sns.kdeplot(wdz,  color=col, ax=ax4z,  marker=marker, alpha=alpha1, gridsize=30, label=shortlegend)
                if MolecID==IDshow1: sns.kdeplot(wdz,  color=col, ax=ax4t,  marker=marker, alpha=alpha1, gridsize=30, label=shortlegend)
         #       ax4z.containers[0].remove()
            #    sns.histplot(wdz, binwidth=10, binrange=(0,300), element="poly", ax=ax4z, fill=False)
                ax4z.axis([0, 400, 0, 0.02]); ax4z.set_xlabel("Jump distance (nm)")
                if MolecID==IDshow1: ax4t.axis([0, 400, 0, 0.015]); ax4t.set_xlabel("Jump distance (nm)")
                
              #  print(shortlegend)
                (pEq0, dpEq0, kmin, kmax, x2, y2, y2_fit0) = survival(wdur, tmax, countNoOpen, 0, '', marker, mindur=mindur, ax=ax , col=col, shortlegend=shortlegend)
                (pEq0, dpEq0, kmin, kmax, x2, y2, y2_fit0) = survival(wdur, tmax, countNoOpen, 0, '', marker, mindur=mindur, ax=ax2, col=col, shortlegend=shortlegend)
                koff = 1/pEq0; dkoff = dpEq0/pEq0**2; count+=1
                if rapa:
                    (pEq0, dpEq0, kmin, kmax, x2, y2, y2_fit0) = survival(wdurA, tmax, countNoOpen, 0, '', marker, mindur=mindur, ax=ax2A , col=col, shortlegend=shortlegend)
                    koff2A = 1/pEq0; dkoff2A = dpEq0/pEq0**2
                else:
                    koff2A = np.nan; dkoff2A = np.nan
                if rapa:
                    (pEq0, dpEq0, kmin, kmax, x2, y2, y2_fit0) = survival(wdurB, tmax, countNoOpen, 0, '', marker, mindur=mindur, ax=ax2B , col=col, shortlegend=shortlegend)
                ...
                for ax2i in [ax, ax1, ax2, ax2A, ax2B]:
                    ...
                    ax2i.legend(title='ID '+str(MolecID), title_fontsize=fontsize, prop={'size': fontsize}, fontsize=fontsize, loc="upper right")
                ax4z.legend(title='ID '+str(iID+1), title_fontsize=fontsize, prop={'size': fontsize}, fontsize=8)
                if MolecID==IDshow1: ax4t.legend(title='ID '+str(iID+1), title_fontsize=fontsize, prop={'size': fontsize}, fontsize=8)
    ...
    figfitspecname, axfitspecall = plt.subplots(2,2, num='Spectrum fit All', figsize=(2*sizefig, 2*sizefig), dpi=100)
    for ifit, icol in zip([0,1,2], ['r','b','g']):
        dfloc = dffitspec[ (dffitspec['fittype']==ifit) & (dffitspec['MolecID']==IDshow1) ]
  #      axfitspecall[0,0].errorbar(dfloc["power"], dfloc['Dxraw/Dtheo'], dfloc['dDxraw/Dtheo'], fmt=icol+'o-', markerfacecolor='none', label=str(ifit)+' raw', alpha=alpha)
  #      axfitspecall[0,0].errorbar(dfloc["power"], dfloc['Dx/Dtheo'], dfloc['dDx/Dtheo'], fmt=icol+'s-', label=str(ifit)+' bin', alpha=alpha)
        axfitspecall[1,0].errorbar(dfloc["power"], dfloc['kxraw'], dfloc['dkxraw'], fmt=icol+'o-', markerfacecolor='none', label=str(ifit)+' raw', alpha=alpha)
        axfitspecall[1,0].errorbar(dfloc["power"], dfloc['kx'], dfloc['dkx'], fmt=icol+'s-', label=str(ifit)+' bin', alpha=alpha)
  #      axfitspecall[0,1].errorbar(dfloc["power"], dfloc['Dyraw/Dtheo'], dfloc['dDyraw/Dtheo'], fmt=icol+'o-',  markerfacecolor='none', label=str(ifit)+' raw', alpha=alpha)
  #      axfitspecall[0,1].errorbar(dfloc["power"], dfloc['Dy/Dtheo'], dfloc['dDy/Dtheo'], fmt=icol+'s-', label=str(ifit)+' bin', alpha=alpha)
        axfitspecall[1,1].errorbar(dfloc["power"], dfloc['kyraw'], dfloc['dkyraw'], fmt=icol+'o-',  markerfacecolor='none', label=str(ifit)+' raw', alpha=alpha)
        axfitspecall[1,1].errorbar(dfloc["power"], dfloc['ky'], dfloc['dky'], fmt=icol+'s-', label=str(ifit)+' bin', alpha=alpha)   
    for axii in [ axfitspecall[0,0], axfitspecall[1,0], axfitspecall[0,1], axfitspecall[1,1] ]:
        axii.legend(loc='upper left', fontsize=fontsize); axii.set_xlabel('Power (%)'); axii.set_xlim(0,31)
 ...
 #   axfitspecall[0,0].set_ylabel('Dx/Dtheo'); axfitspecall[0,1].set_ylabel('Dy/Dtheo')
    ...
    if SaveGraph:
        ...
        plt.figure('multihistodzjump'); plt.savefig(outpath+'multihistodzjump', transparent=False)
        ...
        plt.figure(figSpectestname); plt.savefig(outpath+figSpectestname, transparent=False)
        plt.figure(figdztestname); plt.savefig(outpath+figdztestname, transparent=False)
...
def KramerxB(wf, k0, xB): return k0/xB**2*np.exp(wf*xB/kBT)
def logKramerxB(wf, k0, xB): return np.log(k0/xB**2) + wf*xB/kBT
...
def WLC(x, *p):     # https://en.wikipedia.org/wiki/Worm-like_chain#cite_note-Petrosyan-12
  #  f = (kBT/p[0]) * ( 0.25*(1-x/p[1])**-2 - 0.25 + x/p[1] -0.8*(x/p[1])**2.15 ) # force vs length x; p[1] = L0; p[0] = Lp
    L0 = p[1]
  #  L0 = 1000
    f = (kBT/p[0]) * ( 0.25*(1-x/L0)**-2 - 0.25 + x/L0 ) # force vs length x; p[1] = L0; p[0] = Lp
    return f*(1-x/p[1]>0)

def FJC(f, *p):
    x = p[1]*(1/np.tanh(f*p[0]/4) - 4/(f*p[0]))
    return x

def fitBellFriddleKramers(func, wf, wk, weights=None):
    Hmodel = Model(func)  # Hmodel = Model(Bell)
    Hparams = Hmodel.make_params()
    xBinit = 1.; k0init = 1e-4
    if func == logKramerxB : k0init = 1e-2
...
figname2Bis2 = 'koff0_vs_xBeta2'; fig = plt.figure(figname2Bis2, figsize=(6,6), dpi=100); ax2Bis2 = plt.gca()
figname2Bis3 = 'D0_vs_xBeta'; fig = plt.figure(figname2Bis3, figsize=(6,6), dpi=100); ax2Bis3 = plt.gca()
figname2Ter = 'kx vs xBeta'; fig = plt.figure(figname2Ter, figsize=(6,6), dpi=100); ax2Ter = plt.gca()
figname2Ter0 = 'Radius vs xbeta'; fig = plt.figure(figname2Ter0, figsize=(6,6), dpi=100); ax2Ter0 = plt.gca()
...
figname2Ter6fjc = 'Lp_vs_L0fjc'; fig = plt.figure(figname2Ter6fjc, figsize=(6,6), dpi=100); ax2Ter6fjc = plt.gca()
...
figsize = (ncol2*sizefig, nrow2*sizefig)
figBis, axallBis = plt.subplots(nrow2, ncol2, num='offrate_vs_forceBis', figsize=figsize, dpi=100)
figBisAB, axallBisAB = plt.subplots(nrow2, ncol2, num='offrate_vs_forceBisAB', figsize=figsize, dpi=100)
...
# figForcetestname = 'Force_test'+str(IDshow); fig = plt.figure(figForcetestname, figsize=(6,6), dpi=100); axPfBtest = plt.gca()
# figAngletestname = 'Angle_test'+str(IDshow); fig = plt.figure(figAngletestname, figsize=(6,6), dpi=100); axPAtest = plt.gca()
figForcetestname = 'Force_test'; fig = plt.figure(figForcetestname, figsize=(6,6), dpi=100); axPfBtest = plt.gca()
figAngletestname = 'Angle_test'; fig = plt.figure(figAngletestname, figsize=(6,6), dpi=100); axPAtest = plt.gca()
figDifftestname = 'Diff_test'; fig = plt.figure(figAngletestname, figsize=(6,6), dpi=100); axPDBtest = plt.gca()

#ax3.errorbar(wfold, wfL, wdfL, wdfold, c=c, label='ID='+str(x), fmt='o', alpha=alpha)
...
for xy in ['x', 'y']:
    ...
    for ii, MolecID in enumerate(np.arange(np.max(ListMolec))+1):# enumerate(ListMolec_rapa): #:
        if rapa:
            listD, listk, listdD, listdk, listL, listR, listdR = GlobalSpecFit(dfstage0, folder_spectra, MolecID, xy, step=2)
        else:
            listD, listk, listdD, listdk, listL, listR, listdR = GlobalSpecFit(dfstage0, folder_spectra, MolecID, xy, step=2)
            listD, listk, listdD, listdk, listL, listR0, listdR0 = GlobalSpecFit(dfstage0, folder_spectra, MolecID, xy, step=2, listD=listD)
...
for xy in ['x', 'y']: dfstage['Rprec'+xy] = findR(dfstage['Lglobal'], dfstage['D'+xy+'global'])

wlogk0 = np.ndarray(Nmolec); wdlogk0 = np.ndarray(Nmolec)   # log koff at 0 force Bell Fit
wk0 = np.ndarray(Nmolec); wdk0 = np.ndarray(Nmolec)
...
wxBk = np.ndarray(Nmolec); wdxBk = np.ndarray(Nmolec)   # Kramer model
wD0k = np.ndarray(Nmolec); wdD0k = np.ndarray(Nmolec)   # Kramer model
...
wbFJC = np.zeros(Nmolec); wdbFJC = np.zeros(Nmolec)
wLFJC = np.zeros(Nmolec); wdLFJC = np.zeros(Nmolec)
...
if SelectMolecule:
    for ix, x in enumerate(ListMolec):
        ...
        BeadRad_ix = dfx['Rx']
        BeadRad_iy = dfx['Ry']
        BeadRad_i = (BeadRad_ix+BeadRad_iy)/2
   ...
   #     wlP = dfx['p1Lo_D']; wdlP = dfx['p2Lo_D']
        ...
        wkCyclex = wFCyclex/(wlco+BeadRad_ix)
        wkCycley = wFCycley/(wlco+BeadRad_iy)
   ...
   #     wlcalP = wlgP
        ...
        wfxgP = wkxgP*(wlgP+BeadRad_ix); wdfxgP = wdkxgP*(wlgP+BeadRad_ix)
        wfygP = wkygP*(wlgP+BeadRad_iy); wdfygP = wdkygP*(wlgP+BeadRad_iy) 
  ...
  #      print(wkxgP); print(wlgP); print(BeadRad_i)
   #     print(list(zip(wkxgP,wlgP,BeadRad_i)))
        ...
        wfxcalP = wkxP*(wlcalP+BeadRad_ix)
        wfycalP = wkyP*(wlcalP+BeadRad_iy)
        ...
        waPcorr = 180/np.pi*np.arcsin(wlP/(BeadRad_i+wlP)*np.sin(waP*np.pi/180))    # correction for equation theta (wrong in AFS_JDNA4.57)
        wfPproj = wfP/np.cos(waPcorr*np.pi/180)
        ...
        axPf.errorbar(wP, wfP, c=c, fmt=symb+'-', label='F vertical', alpha=alpha) 
        axPf.errorbar(wP, wfPproj, c=c, fmt='s-', markerfacecolor='none', label='F projected', alpha=alpha) 
        axPf.errorbar(wP, wfxcalP, c='k', fmt='+', label='Fx direct cal') 
        axPf.errorbar(wP, wfycalP, c='k', fmt='x', label='Fy direct cal') 
        axPf.errorbar(wP, wfL, c=c, fmt='*--', label='F Lumicks', alpha=alpha) 
        axPf.errorbar(wP, wFCyclex, c='k', fmt='h-.', markerfacecolor='none', label='Fcyclex', alpha=alpha) 
        axPf.errorbar(wP, wFCycley, c='k', fmt='d-.', markerfacecolor='none', label='Fcycley', alpha=alpha) 
        axPf.errorbar(wP, wFSDx_c, c='k', fmt='o-.', label='FSDx_c', alpha=alpha) 
        axPf.errorbar(wP, wFSDy_c, c='k', fmt='s-.', label='FSDy_c', alpha=alpha) 
        ...
        if (x in IDshow):
            axPfBtest.errorbar(wP-DeltaP, wfxgP, wdfxgP, c=c, fmt=symb+'-', label='ID '+str(ix+1)+' x') 
            axPfBtest.errorbar(wP+DeltaP, wfygP, wdfygP, c=c, fmt=symb+'-', markerfacecolor='none', label='ID '+str(ix+1)+' y')
        
        axPL.errorbar(wP, wlcalP, c=c, fmt=symb+'-', label='L median anchor point', alpha=alpha)
        if not rapa: axPL.errorbar(wP, wlgP, c=c, fmt=symb+'-', label='L cycle', alpha=alpha)
        axPL.errorbar(wP, wlCcalP, c=c, fmt=symb+'-', label='L contour anchor point', markerfacecolor='none', alpha=alpha)
        
        if (x in IDshow): axPAtest.errorbar(wP, waPcorr, c=c, fmt=symb+'-', label='ID '+str(ix+1), alpha=alpha) 
        axPA.errorbar(wP, waPcorr, c=c, fmt=symb+'-', label='From Median', alpha=alpha) 
   #     axPA.errorbar(wP, waCP, c=c, fmt='s-', markerfacecolor='none', label='From Contour', alpha=alpha) 
    #    axPA.errorbar(wP, waP, c=c, fmt='s-', markerfacecolor='none', label='From Contour', alpha=alpha) 
        axPA.axhline(y=waPcorr.mean(), color='k', linestyle='--', label='Mean Angle')
        
        axPk.errorbar(wP, wkxP, c=c, fmt=symb+'-', label='kx', alpha=alpha) 
        axPk.errorbar(wP, wkyP, c=c, fmt='s-', markerfacecolor='none', label='ky', alpha=alpha)
        axPk.errorbar(wP, wkxPcorr, c='k', fmt=symb+'-', label='kxcorr', alpha=alpha) 
        axPk.errorbar(wP, wkyPcorr, c='k', fmt='s-', markerfacecolor='none', label='kycorr', alpha=alpha) 
        axPk.errorbar(wP, wkCyclex, c='k', fmt='+', label='kcyclex', alpha=alpha)
        axPk.errorbar(wP, wkCycley, c='k', fmt='x', label='kcycley', alpha=alpha)
        
        axPkB.errorbar(wP, wkxgP, wdkxgP, c=c, fmt=symb+'-', label='kxg', alpha=alpha)
        axPkB.errorbar(wP, wkygP, wdkygP, c=c, fmt=symb+'-', markerfacecolor='none', label='kyg', alpha=alpha)
        axPkB.errorbar(wP, wkxrPB, c=c, fmt='s--', label='kxrB', alpha=alpha)
        axPkB.errorbar(wP, wkyrPB, c=c, fmt='s--', markerfacecolor='none', label='kyrB', alpha=alpha)
        axPkB.errorbar(wP, wkxPB, c='k', fmt='x', label='kxB', alpha=alpha)
        axPkB.errorbar(wP, wkyPB, c='k', fmt='+', label='kyB', alpha=alpha)
        axPkT.errorbar(wP-DeltaP, wkxgP, wdkxgP, c=c, fmt=symb+'-', label='kxg', alpha=alpha)
        axPkT.errorbar(wP+DeltaP, wkygP, wdkygP, c=c, fmt=symb+'-', markerfacecolor='none', label='kyg', alpha=alpha)
        
        axPD.errorbar(wP, wDxP, c=c, fmt=symb+'-', label='Dx', alpha=alpha) 
        axPD.errorbar(wP, wDyP, c=c, fmt='s-', markerfacecolor='none', label='Dy', alpha=alpha) 
        axPD.errorbar(wP, wDxytheoP, c='k', fmt='-', label='Dtheo', alpha=alpha)
        axPD.errorbar(wP, wDCyclex, c='k', fmt='+', label='Dcyclex', alpha=alpha) 
        axPD.errorbar(wP, wDCycley, c='k', fmt='x', label='Dcycley', alpha=alpha)
        ...
        # axPDB.errorbar(wP, wDxgP*1e-6, c=c, fmt='+-', label='Dxg', alpha=alpha) 
        # axPDB.errorbar(wP, wDygP*1e-6, c=c, fmt='d-', markerfacecolor='none', label='Dyg', alpha=alpha)    
  ...
  #      print(wfc, wfmax_Hz) 
        iPok = wfc < wfmax_Hz/2
  #      iPok = wDxPB>0
        # axPDB.errorbar(wP[iPok], wDxP[iPok], c=c, fmt=symb+'-', label='Dx', alpha=alpha) 
        # axPDB.errorbar(wP[iPok], wDyP[iPok], c=c, fmt='s-', markerfacecolor='none', label='Dy', alpha=alpha)
        axPDB.errorbar(wP[iPok], wDxrPB[iPok]*1e-6, c=c, fmt=symb+'-', label='Dx', alpha=alpha) 
        axPDB.errorbar(wP[iPok], wDyrPB[iPok]*1e-6, c=c, fmt='s-', markerfacecolor='none', label='Dy', alpha=alpha)    
        # axPDB.errorbar(wP[iPok], wDxPB[iPok]*1e-6, c=c, fmt='+-', label='Dxr', alpha=alpha) 
        # axPDB.errorbar(wP[iPok], wDyPB[iPok]*1e-6, c=c, fmt='d-', markerfacecolor='none', label='Dyr', alpha=alpha)    
        
        axPP.errorbar(wP, wPlateauxS, c=c, fmt=symb+'-', label='Plateau x S', alpha=alpha) 
        axPP.errorbar(wP, wPlateauyS, c=c, fmt=symb+'-', markerfacecolor='none', label='Plateau y S', alpha=alpha) 
        axPP.errorbar(wP, wPlateauxrS, c='k', fmt='+', label='Plateau xr S', alpha=alpha) 
        axPP.errorbar(wP, wPlateauyrS, c='k', fmt='x', label='Plateau yr S', alpha=alpha) 
        axPP.errorbar(wP, wPlateauxg, c='k', fmt='+-', label='Plateau xg', alpha=alpha) 
        axPP.errorbar(wP, wPlateauyg, c='k', fmt='x-', label='Plateau yg', alpha=alpha) 
    ...
    #    axPD.fill_between(np.arange(35), Dmin, Dmax, alpha=0.2)
        ...
        axPD.errorbar(wP[selDavg], wDavg[selDavg], c='k', fmt='s', label='avg D sel', alpha=alpha)       
        axPk.errorbar(wP[selDavg], wkavg[selDavg], c='k', fmt='s', label='from D sel', alpha=alpha)       
        axPf.errorbar(wP[selDavg], wFavg[selDavg], c='k', fmt='s', label='from D sel', alpha=alpha)
  ...
  #      wfv = dfx['Force_pN']; wdfv = dfx['dForce_pN']
   ...
   #     wlP = dfx['Length_calibration (nm)']; wdlP = dfx['error of length']
        ...
        Rxy = ( dfx['Rx'].mean() + dfx['Ry'].mean() ) /2
        
        wacorr = 180/np.pi*np.arcsin(wlD/(BeadRad_i+wlD)*np.sin(wa*np.pi/180)) 
        wfproj = wf/np.cos(wacorr*np.pi/180)
 ...
 #       selkoff = ~np.isnan(wk)[np.append(selLowF, np.zeros(len(wk)-len(selLowF)),dtype='bool')]
        ...
        ax.errorbar(wf[selkoff], wk[selkoff], wdk[selkoff], c=c, fmt=symb+'-', label='ID='+str(x), alpha=alpha1) 
        axBis.errorbar(wf[selkoff], wk[selkoff], wdk[selkoff], c=c, fmt=symb+'-', label='F Dsel', alpha=alpha1) 
        axBis.errorbar(wfproj[selkoff], wk[selkoff], wdk[selkoff], wdf[selkoff], c=c, fmt=symb+'-', markerfacecolor='none', label='F projected', alpha=alpha1) 
        axBisAB.errorbar(wf[selkoff], wk2A[selkoff], wdk2A[selkoff], c='g', fmt='o-', label='ID='+str(x), alpha=alpha) 
        axBisAB.errorbar(wf[selkoff], wk2B[selkoff], wdk2B[selkoff], c='r', fmt='o-', label='ID='+str(x), alpha=alpha) 
        axMat.errorbar(wk2A[selkoff], wk2B[selkoff], c=c, fmt='o')
        axOtheFlyDx.scatter(wDxP[selkoff], wDCyclex[selkoff], c=np.log(wkxP[selkoff]/0.1), label='Dx', cmap='gray', alpha=alpha)
        axOtheFlyDy.scatter(wDyP[selkoff], wDCycley[selkoff], c=np.log(wkxP[selkoff]/0.1), label='Dy', cmap='gray', alpha=alpha)
        axOtheFlykx.scatter(wkxP[selkoff], wkCyclex[selkoff], c=np.log(wkxP[selkoff]/0.1), label='kx', cmap='gray', alpha=alpha)
        axOtheFlyky.scatter(wkyP[selkoff], wkCycley[selkoff], c=np.log(wkxP[selkoff]/0.1), label='ky', cmap='gray', alpha=alpha)
        axOtheFlySDx.scatter(wFSDx[selkoff], wFSDx_c[selkoff], c=c, label='FSDx', alpha=alpha)
        axOtheFlySDy.scatter(wFSDy[selkoff], wFSDy_c[selkoff], c=c, label='FSDy', alpha=alpha)
        axContour.scatter(wlcalP, wlCcalP, c=c, label='Open length', alpha=alpha)
        axXYk.scatter(wkxP[selkoff], wkyP[selkoff], c=c, label='XYkcal', alpha=alpha)
        axXYD.scatter(wDxP[selkoff], wDyP[selkoff], c=c, label='XYDcal', alpha=alpha)
        axXYF.scatter(wfxcalP[selkoff], wfycalP[selkoff], c=c, label='XYFcal', alpha=alpha)

        axA.errorbar(wf[~np.isnan(wf)], wa[~np.isnan(wf)], c=c, fmt=symb+'-', label='From Median', alpha=alpha) 
        axA.errorbar(wf[~np.isnan(wf)], waC[~np.isnan(wf)], c=c, fmt=symb+'-', markerfacecolor='none', label='From Contour', alpha=alpha) 
        ...
        axL.errorbar(wf[nonan_f_l], wl[nonan_f_l], wdl[nonan_f_l], wdf[nonan_f_l], c=c, fmt=symb+'-', label='L calibration', alpha=alpha) 
   #     axL.errorbar(wfproj[nonan_f_l], wl[nonan_f_l], wdl[nonan_f_l], wdf[nonan_f_l], c=c, fmt=symb+'-', markerfacecolor='none', label='F projected', alpha=alpha) 
        axL.errorbar(wf[nonan_f_l], wlcal[nonan_f_l], c='k', fmt='+', label='L calib', alpha=alpha) 
        axL.errorbar(wf[nonan_f_l], wlCcal[nonan_f_l], c='k', fmt='x', label='LC calib', alpha=alpha) 
        axL.errorbar(wf[nonan_f_l], wlcc[nonan_f_l], wdlcc[nonan_f_l], wdf[nonan_f_l], c=c, fmt='s-', markerfacecolor='none', label='LC closed', alpha=alpha) 
        axL.errorbar(wf[nonan_f_l], wlco[nonan_f_l], wdlco[nonan_f_l], wdf[nonan_f_l], c=c, fmt='o-', markerfacecolor='none', label='LC open', alpha=alpha) 
        axLB.errorbar(wf, wlD, wdlD, wdf, c=c, fmt=symb+'-', label='L Direct cal', alpha=alpha) 
        axLB.errorbar(wf, dfx['Lglobal'], c=c, fmt=symb+'-', markerfacecolor='none', label='L spec cal', alpha=alpha) 
        
        axNC.errorbar(wf, wnc, c=c, fmt=symb+'-', label='ID='+str(x), alpha=alpha) 
        axFL.errorbar(wf, wfL, wdfL, wdf, c=c, fmt=symb+'-', label='ID='+str(x), alpha=alpha) 
        axFL.plot(wfL, wfL, c='k', linestyle='-', label='ID='+str(x), alpha=alpha1) 

        axDZ.errorbar(wf, wDZ, wdDZ, wdf, c=c, fmt=symb+'-', label='ID='+str(x), alpha=alpha) 
        axDZ.errorbar(wf, wDL, wdDL, wdf, c=c, fmt=symb+'-', markerfacecolor='none', label='ID='+str(x), alpha=alpha) 
        axLZ.errorbar(wl, wDZ, wdDZ, wdlcc, c=c, fmt=symb+'-', label='ID='+str(x), alpha=alpha) 
        def WLCL0pen(x, *p):
   ...
   ###################  Fits for Length  #############"                   
        if sum(nonan_f_l)>3:
            ...
            axL.plot(wffit, wlfit, linestyle='-.', c='k', label='Affine fit', alpha=alpha)
            L0guess = wlD[nonan_f_lD].max()+100;
            while True:
                try:
                    pEqFJC, pcovEqFJC = curve_fit(FJC, wf[nonan_f_lD], wlD[nonan_f_lD], p0=[20, L0guess])
                    break
                except RuntimeError:
                    print("No convergence FJC"); pEqFJC=[np.nan,np.nan]; pcovEqFJC=[[np.nan, np.nan],[np.nan, np.nan]]
                    break
            L0guess = pEqFJC[1]
            while True:
                try:
                    def WLC_L0(x, *p):
                        ...
                        f = (kBT/p[0]) * ( 0.25*(1-x/L0)**-2 - 0.25 + x/L0 ) # force vs length x; p[1] = L0; p[0] = Lp
                        return f*(1-x/L0>0)
                #    pEqWLC, pcovEqWLC = curve_fit(WLC, wlD[nonan_f_lD], wf[nonan_f_lD], p0=[20, L0guess])
                    pEqWLC_, pcovEqWLC_ = curve_fit(WLC_L0, wlD[nonan_f_lD], wf[nonan_f_lD], p0=[20])
                    break
                except RuntimeError:
                    print("No convergence WLC"); pEqWLC=[np.nan,np.nan]; pcovEqWLC=[[np.nan, np.nan],[np.nan, np.nan]]
                    break
            wlfitWLC = 20*(np.arange(100)+1)
       #     wffitWLC = WLC(wlfitWLC, pEqWLC[0], pEqWLC[1])
            wffitWLC_ = WLC_L0(wlfitWLC, pEqWLC_[0])
       #     wffitWLC_30 = WLC(wlfitWLC, 30, pEqWLC[1])
       #     wffitWLC_50 = WLC(wlfitWLC, 50, pEqWLC[1])
            wffitFJC = (np.arange(50)+0.1)
            wlfitFJC = FJC(wffitFJC, pEqFJC[0], pEqFJC[1])
       #     axL.plot(wffitWLC, wlfitWLC, linestyle='--', c='k', label='WLC fit', alpha=alpha)
     #       axLB.plot(wffitWLC_, wlfitWLC, c='k', label='WLC fit', alpha=alpha)
     #       axLB.plot(wffitFJC, wlfitFJC, c='r', label='FJC fit', alpha=alpha)
     #       axLB.plot(wffitWLC_30, wlfitWLC, linestyle='--', c='k', label='WLC fit Lp=30', alpha=alpha)
        #    axLB.plot(wffitWLC_50, wlfitWLC, linestyle='--', c='k', label='WLC fit Lp=50', alpha=alpha)
        else: 
            pEqElast2 =[np.nan,np.nan]
            pEqWLC=[np.nan,np.nan]; pcovEqWLC=[[np.nan, np.nan],[np.nan, np.nan]]
            pEqFJC=[np.nan,np.nan]; pcovEqFJC=[[np.nan, np.nan],[np.nan, np.nan]]
        ...
        wLp[ix] = pEqWLC_[0] ; wdLp[ix]=np.sqrt(np.diag(pcovEqWLC_))[0]**0.5 
        wL0[ix]= L0guess ; wdL0[ix]=0
        wbFJC[ix] = pEqFJC[0] ; wdbFJC[ix]=np.sqrt(np.diag(pcovEqFJC))[0]**0.5 
        wLFJC[ix]=pEqFJC[1] ; wdLFJC[ix]=np.sqrt(np.diag(pcovEqFJC))[1]**0.5
   ...
   ###################  Fits for Off-rates  #############"                           
        ...
        if len(wFavg[selkoff])>=3:
            ...
            (pEqB, eEqB, res) = fitBellFriddleKramers(logBellxB, wf[selkoff], wk[selkoff])#, 1/sd)
            wlEqB = BellxB(wft, pEqB[0], pEqB[1])
            axBis.errorbar(wft, wlEqB, c='k', fmt='-', label='fit Bell', alpha=alpha)
            
            (pEqBp, eEqBp, resp) = fitBellFriddleKramers(logBellxB, wfproj[selkoff], wk[selkoff])#, 1/sd)
            wlEqBp = BellxB(wft, pEqBp[0], pEqBp[1])       
            axBis.errorbar(wft, wlEqBp, c='k', fmt='--', label='fit Bell proj', alpha=alpha)
            
            (pEqBk, eEqBk, resk) = fitBellFriddleKramers(logKramerxB, wf[selkoff], wk[selkoff])#, 1/sd)
            wlEqBk = KramerxB(wft, pEqBk[0], pEqBk[1])
     #       axBis.errorbar(wft, wlEqBk, c='r', fmt='--', label='fit Kramer', alpha=alpha)
            
            (pEqF, eEqF, resF) = fitBellFriddleKramers(logFriddle, wf[selkoff], wk[selkoff])#, 1/sd)
        ...
        D0k = pEqBk[0] ; dD0k = eEqBk[0]
        xBk = pEqBk[1]; dxBk = eEqBk[1]
        wxBk[ix] = xBk; wdxBk[ix] = dxBk
        wD0k[ix] = D0k; wdD0k[ix] = dD0k
        if k0>0:
            wlogk0[ix] = np.log(k0); wdlogk0[ix] = dk0/k0
            wk0[ix] = k0; wdk0[ix] = dk0
        else:
            wlogk0[ix] = np.nan; wdlogk0[ix] = np.nan
            wk0[ix] = np.nan; wdk0[ix] = np.nan
        if k0p>0:    
        ...
        print(x, 'Fit Kramer    Bell' , '(', len(wk[selkoff]) , ' points):  D0=  %.6f' % D0k,'+/-  %.6f' % dD0k,  ' xB= %.2f'% xBk,'+/- %.2f' % dxBk, 'fres =%.2f' % fresk)
        print(x, 'Fitting Molec Frid' , '(', len(wk[selkoff]) , ' points):  k0=  %.6f' % k0F,'+/-  %.6f' % dk0F,  ' xB= %.2f'% xBF,'+/- %.2f' % dxBF, 'kc =%.2f' % pEqF[2])

     #   axRR.scatter(dfx['R'+xy][0], slopePf**(1/3), c=cFOV, alpha=alpha)
        axRR.scatter(dfx['Rprec'+xy][0], slopePf**(1/3), c=cFOV, marker='s', alpha=alpha)
        axRR1.scatter(dfx['Rprec'+xy][0], wl[nonan_f_l].median(), c=cFOV, alpha=alpha)

        if fB>0:
            ax2.errorbar(wfB[ix], np.exp(wlogk0[ix]), dk0, wdfB[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2.errorbar(wfBF[ix], np.exp(wlogk0F[ix]), dk0F, wdfBF[ix], c=c, fmt=symb, markerfacecolor='none', alpha=alpha)
            ax2Bis.errorbar(wxB[ix], np.exp(wlogk0[ix]), dk0, wdxB[ix], c=c, label='ID='+str(x), fmt=symb, markerfacecolor='none', alpha=alpha)
            ax2Bis3.errorbar(wxBp[ix], wD0k[ix], wdD0k[ix], wdxB[ix], c=c, label='ID='+str(x), fmt=symb, markerfacecolor='none', alpha=alpha)
            ax2Bis2.errorbar(1/(wxB[ix]**2), np.exp(wlogk0[ix]), dk0, wdxB[ix]/(wxB[ix]**3), c=c, label='ID='+str(x), fmt=symb, markerfacecolor='none', alpha=alpha)
            ax2Ter.errorbar(wxB[ix], wkx[ix], wdkx[ix], wdxB[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter0.errorbar(wxB[ix], Rxy, 0, wdxBp[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter1.errorbar(wxB[ix], wlmax[ix], wdlmax[ix], wdxBp[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter2.errorbar(wfB[ix], wkz[ix], wdkz[ix], wdfB[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter3.errorbar(wkx[ix], wkz[ix], wdkz[ix], wdkx[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter4.errorbar(wLp[ix], wkz[ix], wdkz[ix], wdLp[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter6.errorbar(wL0[ix], wLp[ix], wdLp[ix], wdL0[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter6fjc.errorbar(wLFJC[ix], wbFJC[ix], wdbFJC[ix], wdLFJC[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter5.errorbar(wfB[ix], wLp[ix], wdLp[ix], wdfB[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax2Ter7.errorbar(wlrange[ix], wdzrange[ix], wddzrange[ix], wdlrange[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax4.errorbar(wfB[ix], wfrange[ix], wdfrange[ix], wdfB[ix], c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
            ax3.errorbar(wf, wfL, wdfL, wdf, c=c, label='ID='+str(x), fmt=symb, alpha=alpha)
        ...
        axA.set_ylabel(r'Pulling Angle $\theta$ (deg)'); axA.set_ylim(0,60); axA.set_xlabel('Force')
        ...
        axPfB.set_ylabel('Force (pN)'); axPfBtest.set_ylabel('Force (pN)')
        if not rapa: axPfB.set_ylim(0,40); axPfBtest.set_ylim(0,40)
        ...
        axPA.set_ylabel(r'Pulling Angle $\theta$ (deg)'); axPA.set_ylim(0,35)
        axPAtest.set_ylabel(r'Pulling Angle $\theta$ (deg)'); axPAtest.set_ylim(0,35)
        axPAtest.set_ylabel(r'Pulling Angle $\theta$ (deg)'); axPAtest.set_ylim(0,35)
        axPP.set_ylabel('Plateau PSD'); axPP.set_ylim(1e4, 1e10); axPP.set_yscale('log')
        if rapa: axPf.set_ylim(0,25)
        if rapa:
            axPfB.set_ylim(0,25); axPfBtest.set_ylim(0,25)
        ...
        for axBisi in [axBis, axBisAB]:
            axBisi.set_ylabel('Dissociation rate $k_{off}$ (1/s)'); axBisi.set_yscale('log')
            axBisi.set_ylim(1e-5,0.2)
            if rapa: axBisi.set_ylim(0.01, 0.4)
        Annotation = 'ID '+str(ix+1)  #  'ID '+str(x)+' FOV '+str(wfov[ix])
        for axBisi in [axBis, axBisAB, axL, axLB, axA, axNC, axFL, axPf, axPL, axPfB, axPk, axPkB, axPkT, axPD, axPDB, axPA, axPP, axDZ, axLZ]:
         ...
         #       res = res + "\n WLC L0={:1.0f}".format(wL0[ix])+" Lp={:1.0f}".format(wLp[ix])
          #      res = res + "\n FJC L0={:1.0f}".format(wLFJC[ix])+" b={:1.0f}".format(wbFJC[ix])
                axBisi.text(0.5, 1600, Annotation+':'+res , size=fontsize)
            elif axBisi == axNC or axBisi == axFL  or axBisi == axPf or axBisi == axPfB or axBisi == axDZ or axBisi == axA:
                if axBisi == axPf and len(wfP[~np.isnan(wfP)])>2: 
                    ...
                    axBisi.text(1, 8, text, size=fontsize)
                    wCoefPow_F[ix] = slope
              #      axBisi.text(80, 0.5, TextLinear(wfP, wP), size=10)
      #          if ShowAnnotation: axBisi.text(1, 5, 'ID '+str(x)+' FOV '+str(wfov[ix]) , size=fontsize)
                if ShowAnnotation: axBisi.text(1, 5, Annotation , size=fontsize)
            elif axBisi == axLZ:
                axBisi.text(700, 110, Annotation , size=fontsize)
            elif axBisi == axBis or axBisi == axBisAB:
                res = " k0 ={:1.6f}".format(k0)+" xB ={:1.2f}".format(xB)
                res += "\n k0p={:1.6f}".format(k0p)+" xBp={:1.2f}".format(xBp)
          #      res += "\n D0k={:1.6f}".format(D0k)+" xBp={:1.2f}".format(xBk)
                ypos = 0.01 if rapa else 0.02
                axBisi.text(0.5, ypos, Annotation+'\n'+res , size=fontsize)
            elif axBisi == axPk or axBisi == axPkB or axBisi == axPkT:
                axBisi.text(0.5, 0.008, Annotation , size=fontsize)
            elif axBisi == axPD or axBisi == axPDB:
                axBisi.text(0.5, 0.05, Annotation , size=fontsize)
            elif axBisi == axPP:
                axBisi.text(0.5, 1e5, Annotation , size=fontsize)
            elif axBisi == axPL:
                axBisi.text(0.5, 1500, Annotation , size=fontsize)
            elif axBisi == axNC:
                axBisi.text(0.5, 0.5, Annotation , size=fontsize)
            elif axBisi == axPA:
                axBisi.text(0.5, 30, Annotation , size=fontsize)
            ...
            if (x in IDshow): axPfBtest.set_xlabel('% High power'); axPfBtest.set_xlim(0, HighPowerMax)
            if (x in IDshow): axPAtest.set_xlabel('% High power'); axPAtest.set_xlim(0, HighPowerMax)
            if (x in IDshow): axPfBtest.legend(fontsize=fontsize); axPAtest.legend(fontsize=fontsize)
            if ix%ncol2!=0: axBisi.axes.get_yaxis().set_visible(False)
            if ix//ncol2!=nrow2-1: axBisi.axes.get_xaxis().set_visible(False)
        if ix==0:
  #          axBis.legend(loc="upper left", fontsize=fontsize)
 #           for axii in [axPf, axPL, axPfB, axPA, axPP, axPD, axPDB, axPk, axPkB, axPkT, axL, axLB, axA]: axii.legend(fontsize=fontsize)
            for axii in [axPf, axPL, axPP, axPD, axPDB, axPk, axPkB, axL, axA]: axii.legend(fontsize=fontsize)
        ...
        axMat.set_xlabel('Early $k_{off}$ (1/s)'); axMat.set_ylabel('Late $k_{off}$ (1/s)')
...
if rapa:
    Rmin = 700; Rmax = 1200; dR = 50
else:
    Rmin = 600; Rmax = 2000; dR = 100
figname4bxy = 'RDiffusionHistxy'; fig = plt.figure(figname4bxy, figsize=(3,3), dpi=100); ax4bxy = plt.gca()
for huemapxy , xy in zip(['r', 'b'] , ['x', 'y']): #['precx', 'precy', 'x', 'y']:   
    figname4a = 'RDiffusion'+xy; fig = plt.figure(figname4a, figsize=(max(ListMolec)*0.25,6), dpi=100); ax4a = plt.gca()
    figname4b = 'RDiffusionHist'+xy; fig = plt.figure(figname4b, figsize=(3,3), dpi=100); ax4b = plt.gca()
 #   sns.boxplot(x="Molecule ID", y='R'+xy, data=dfstage, ax=ax4a)
    ListMolec_str = [str(ID)  for ID in ListMolec]
    ListRxy = [dfstage.loc[dfstage['Molecule ID'] == ID]['R'+xy].mean()  for ID in ListMolec]
 ...
 #   ax4a.scatter(dfstage['Molecule ID'], dfstage['R'+xy], c='k', marker='x', label='Measured')
    sns.histplot(ListRxy, binwidth=dR, ax=ax4b)
    sns.histplot(ListRxy, binwidth=dR, element="step", fill=False, ax=ax4b)
    ax4bxy.hist(ListRxy, bins=int((Rmax-Rmin)/dR), range=(Rmin, Rmax), color=huemapxy, label=xy, alpha=alpha1)
    for Ri in np.unique(ListBeadR):
        ax4b.axvline(x=Ri, color='r', linestyle='-.', label='Nominal')
        if xy=='x': ax4bxy.axvline(x=Ri, color='r', linestyle='-.', label='Nominal')
 ...
 #   ax4a.scatter(dfstage['Molecule ID'], dfstage['Bead radius (nm)'], c='r', marker='+', label="Nominal")
    ...
    ax4a.set_xlabel('Molecule ID'); ax4a.set_ylabel('Fitted Bead Radius '+xy+' (nm)'); ax4a.set_ylim(Rmin, Rmax)
    ax4b.set_ylabel('Count'); ax4b.set_xlabel('Fitted Bead Radius '+xy+' (nm)'); ax4b.set_xlim(Rmin, Rmax)
    ax4bxy.set_ylabel('Count'); ax4bxy.set_xlabel('Fitted Bead Radius $R$ (nm)'); ax4bxy.set_xlim(Rmin, Rmax)
    ax4a.legend(loc="upper right", fontsize=fontsize)
    ax4bxy.legend(loc="upper right", fontsize=fontsize)
    if SaveGraph: plt.figure(figname4a); plt.tight_layout(); plt.savefig(outpath+figname4a, transparent=False)
    if SaveGraph: plt.figure(figname4b); plt.tight_layout(); plt.savefig(outpath+figname4b, transparent=False)
    

if SaveGraph: plt.figure(figname4bxy); plt.tight_layout(); plt.savefig(outpath+figname4bxy, transparent=False)

ax.set_xlabel('Force (pN)'); ax.set_ylabel('$k_{off}$ (1/s)'); ax.set_yscale('log')
ax.set_ylim(1e-4,0.1); ax.set_xlim(0,35)
...
if sum(nonan)>=2:
    ...
    ax2.plot(kBT/wxBfit, np.exp(wlogk0_fit), linestyle='-.', c='k', label='Fit', alpha=alpha)
    ax4.text(1.5, 13, TextCorrel(wfrange, wfB), size=fontsize)
    ax2Ter.text(1.5, 0.005, TextCorrel(wxB, wkx), size=fontsize)
    ax2Ter0.text(1.5, 700, TextCorrel(wxB, wRxy), size=fontsize)
    ax2Ter1.text(1.5, 700, TextCorrel(wxB, wlmax), size=fontsize)
    ax2Ter2.text(1.5, 0.04, TextCorrel(wfB, wkz), size=fontsize)
    ax2Ter3.text(0.003, 0.01, TextCorrel(wkx, wkz), size=fontsize)
    ax2Ter4.text(30, 0.01, TextCorrel(wLp, wkz), size=fontsize)
    ax2Ter5.text(1.5, 30, TextCorrel(wfB, wLp), size=fontsize)
 #   ax2Ter6.text(1400, 50, TextCorrel(wL0, wLp), size=10)
    ax2Ter7.text(800, 125, TextCorrel(wlrange, wdzrange), size=fontsize)
else:
    print('No enough common data on xB and k0')


for ax2i in [ax2, ax2Bis, ax2Bis2]:
    ...
    ax2i.set_ylim(1e-7,1e-1)
    if ax2i==ax2: ax2i.set_yscale('log')
    ax2i.set_xlim(0,10)
    if rapa: ax2i.set_ylim(0.0,0.07); ax2.set_xlim(0,MaxForce+5)
 #   ax2Bis3.set_ylabel('D0 (nm$^2$/s)');

if rapa:
    def Kramer(x, *p): return p[0]/(x*x)
    def Kramer2(x, *p): return p[0]*x
    wxB_ = np.array(wxB); #ix = (wxB_>0.15) & (wxB<0.6); wxB_ = wxB_[ix]
    wk0_ = np.array(wk0); #wlogk0_ = wlogk0_[ix]
    wdk0 = np.array(wdk0)
    while True:
        try:
     #       pEqKramer, pcovEqKramer = curve_fit(Kramer, wxB_, np.exp(wlogk0_), p0=[0.01])
            pEqKramer, pcovEqKramer = curve_fit(Kramer2, 1/(wxB_**2), wk0_, sigma = wdk0, p0=[0.01])
     #       pEqKramer, pcovEqKramer = curve_fit(Kramer2, wxB_, np.exp(wlogk0), p0=[0.01, 1])
            break
        except RuntimeError:
            print("No convergence Kramer"); pEqKramer=[np.nan]; pcovEqKramer=[[np.nan],[np.nan]]
          #  print("No convergence Kramer"); pEqKramer=[np.nan, np.nan]; pcovEqKramer=[[np.nan, np.nan],[np.nan, np.nan]]
            break
    c = pEqKramer[0]; EE = 25; DD = c/2*(1/EE)*(np.pi/EE)*np.exp(EE)
    print('Fit Kramer c (nm2/s)=', c,'+/-', pcovEqKramer[0][0],  ' DD (µm²/s)=', DD*1e-6)
    print('Pearson', pearsonr(1/(wxB_**2), wk0_))
    wxbfitKramer = (np.arange(50)+0.2)*0.02
    wxbfitKramer2 = (np.arange(100))
    wkoff0fitKramer2 = Kramer2(wxbfitKramer2, pEqKramer[0])
    #wkoff0fitKramer = Kramer2(wxbfitKramer, pEqKramer[0], pEqKramer[1])
 #   ax2Bis.plot(wxbfitKramer, wkoff0fitKramer, c='k', alpha=0.5)
    ax2Bis2.plot(wxbfitKramer2, wkoff0fitKramer2, c='k', linestyle='--', alpha=0.5)
    
    
ax2.set_xlabel('Bell Force (pN)')
if rapa:
    ax2Bis.set_xlim(0,1.0); ax2Bis2.set_xlim(0,100); ax2Bis3.set_xlim(0,1.0);
else: 
    ax2Bis.set_xlim(0,1.5); ax2Bis2.set_xlim(0,50)
ax2Bis.set_xlabel('Well-barrier distance $x_b$ (nm)'); ax2Bis.set_ylabel('off-rate at zero force $k^0_{off}$ (1/s)')
ax2Bis3.set_xlabel('Well-barrier distance $x_b$ (nm)'); ax2Bis3.set_ylabel('D0 (nm$^2$/s)')
ax2Bis2.set_xlabel('$1/x_b^2$ (nm$^{-2}$)'); ax2Bis2.set_ylabel('off-rate at zero force $k^0_{off}$ (1/s)')
ax2Ter.set_xlim(0,3); ax2Ter.set_xlabel('Distance Barrier (nm)'); ax2Ter.set_ylabel('kx DNA (pN/nm)')
if rapa: ax2Ter0.set_xlim(0,1)
if not rapa: ax2Ter0.set_xlim(0,3)
ax2Ter0.set_xlabel('Distance Barrier (nm)'); ax2Ter0.set_ylabel('Bead Radius (nm)'); ax2Ter0.set_ylim(600,2200)
...
ax2Ter6.set_xlim(800,2400); ax2Ter6.set_xlabel('DNA contour length WLC $L_0$ (nm)'); ax2Ter6.set_ylabel('DNA persistence length WLC $L_p$ (nm)'); ax2Ter6.set_ylim(0,40)
ax2Ter6fjc.set_xlim(800,2400); ax2Ter6fjc.set_xlabel('DNA contour length FJC $L$ (nm)'); ax2Ter6fjc.set_ylabel('DNA Kuhn length FJC $b$ (nm)'); ax2Ter6fjc.set_ylim(0,80)
...
if rapa:  
    ...
    for icol, col in enumerate(listcol):
        ...
        axcol.legend(loc="upper right", fontsize=fontsize); axcol.get_legend().remove()
...
listf = MinForce + deltaf*(np.arange((MaxForce-MinForce)//deltaf))
for f in listf:
...
(pEq, eEq, res) = fitBellFriddleKramers(logBellxB, avgf, avgk)#, 1/sd)
...
ax2Bis2.errorbar(1/xBpool**2, k0pool, dk0pool, dxBpool/xBpool**3, fmt='o', c='k', label='Fit Pool')
#ax2Bis.legend(loc="upper right", fontsize=fontsize)
ax2.errorbar(f0pool, k0pool, dk0pool, df0pool, fmt='o', c='k', label='Fit Pool')
for ax2i in [ax2, ax4, ax2Ter, ax2Ter0, ax2Ter1, ax2Ter2, ax2Ter3, ax2Ter4, ax2Ter5, ax2Ter6, ax2Ter6fjc, ax2Ter7, axRR, axRR1]: ax2i.legend(loc="lower right", fontsize=fontsize)
#plt.figure('koff0_vs_xBeta'); plt.tight_layout()
for fignamei in [figname2, figname2Bis, figname2Bis2, figname2Bis3, figname2Ter, figname2Ter0, figname2Ter1, figname2Ter2, figname2Ter3, figname2Ter4, figname2Ter5, figname2Ter6, figname2Ter6+'fjc',
                 ...
                 'Power_Stiffness_Ter', 'Power Friction', 'Power_Friction_Bis', 'Spectrum_test'+str(IDshow), 'dz_test'+str(IDshow1), 'Force_test', 'Angle_test']:
    if SaveGraph: plt.figure(fignamei); plt.savefig(outpath+fignamei, transparent=False)

for fignamei in ['offrate_vs_force', 'koff0_vs_xBeta', 'koff0_vs_xBeta2', 'D0_vs_xBeta', 'Lp_vs_L0', 'Lp_vs_L0fjc', 'Maturation', 'Power_Lengths', 'Force_Lengths_Bis', 'Power_Angles',
                 'No_Close_Fraction', 'Power_vs_Force_Bis', 'Power_Stiffness_Ter', 'Power_Friction_Bis', 'multispectrumx', 'multispectrumy',
                 'multihistodzjump', 'RDiffusionprecx', 'RDiffusionHistx', 'RDiffusionHistxy', 'multisurvival2Bis', 'offrate_vs_forceBis', 'Spectrum_test'+str(IDshow), 'dz_test'+str(IDshow1)]:
    if SaveGraph: plt.figure(fignamei); plt.tight_layout(); plt.savefig(latexpath+fignamei+'_'+AbName, transparent=False)
if SaveGraph: plt.figure( 'Force_test'); plt.savefig(latexpath+ 'Force_test', transparent=True)
if SaveGraph: plt.figure( 'Angle_test'); plt.savefig(latexpath+ 'Angle_test', transparent=True)
